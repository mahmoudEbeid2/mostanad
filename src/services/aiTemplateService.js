...
export const generateHtmlFromDesign = async (filePath, fileName, mimeType) => {
  ...
  const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
    ...
    try {
      ...
      const promptText = `
        You are a world-class Web-to-Print AI. Your task is to identify ONLY the dynamic fill-in values on the provided document and return their exact coordinates and styles as a JSON array. 
        CRITICAL CONTEXT: The document canvas is exactly 1000px wide and 1414px high. The background image of the certificate (including ALL static text, field labels, logos, and borders) is ALREADY present. 
        YOUR EXACT MISSION: Find the actual text values (the dummy data, e.g., "DOXYPHARMA", "09/2028", "TURKEY", "1KG", the test results like "Conforms", etc.) and return their exact bounding boxes and styles as a JSON array. DO NOT convert them into variables. Return the exact original text. DO NOT extract static labels like "PRODUCT NAME", "BATCH NUMBER", "DESCRIPTION", "Company Address". Ignore them completely. 
        JSON SCHEMA REQUIRED: Return a raw JSON array of objects. Do not include markdown formatting. 
        [ 
          { 
      ...
      for (const el of elements) {
        ...
        htmlBuilder += `  <div style="position: absolute; top: ${top}px; left: ${left}px; ${widthStyle} font-size: ${fontSize}px; color: ${color}; background-color: #ffffff; font-weight: ${el.font_weight || 'normal'}; text-align: ${el.text_align || 'left'}; white-space: nowrap; padding: 0 2px;">{{${textToDisplay.replace(/[^a-zA-Z0-9_]/g, '_').toLowerCase()}}}</div>\n`;
      }
